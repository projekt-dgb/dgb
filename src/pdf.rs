...
pub fn export_grundbuch(config: GrundbuchExportConfig) -> Result<(), String> {
    match config.optionen {
        GenerateGrundbuchConfig::EinzelneDatei { ref datei, .. } => {
            let datei = if datei.ends_with(".pdf") {
                datei.clone()
            } else {
                format!("{datei}.pdf")
            };
        ...
        GenerateGrundbuchConfig::MehrereDateien { ref ordner, .. } => export_grundbuch_multi_files(
            config.optionen.get_options(),
            &config.exportiere,
            ordner.clone(),
        ),
    }
...
fn export_grundbuch_single_file(
    ...
    match source {
        ...
        PdfExportTyp::OffenesGrundbuch(gb) => {
            ...
            let (mut doc, page1, layer1) =
                PdfDocument::new(&titel, Mm(210.0), Mm(297.0), "Titelblatt");
        ...
        PdfExportTyp::AlleOffenDigitalisiert(gb) | PdfExportTyp::AlleOffen(gb) => {
            let titel = Path::new(&datei)
                .file_name()
                .map(|f| format!("{}", f.to_str().unwrap_or("")))
                .unwrap_or_default();
            let (mut doc, page1, layer1) =
                PdfDocument::new(&titel, Mm(210.0), Mm(297.0), "Titelblatt");
            let fonts = PdfFonts::new(&mut doc);
            ...
            for f in gb.iter().skip(1) {
                let (titelblatt_page, titelblatt_layer) =
                    doc.add_page(Mm(210.0), Mm(297.0), "Titelblatt");
                write_titelblatt(
                    &mut doc.get_page(titelblatt_page).get_layer(titelblatt_layer),
                    &fonts,
                    &f.titelblatt,
                );
                if options.leere_seite_nach_titelblatt {
...
impl PdfTextRow {
    ...
    fn add_to_page(&self, layer: &mut PdfLayerReference, fonts: &PdfFonts, y_start: f32) {
        ...
        if let Some(s) = self.hvm_exception.as_ref() {
            ...
            layer.add_shape(Line {
                points: vec![
                    ...
                    (
                        Point::new(
                            Mm(x_start_mm as f64),
                            Mm(y_start as f64 - self_height as f64),
                        ),
                        false,
                    ),
                    (
                        Point::new(
                            Mm(x_start_mm as f64 + max_width_mm as f64),
                            Mm(y_start as f64 - self_height as f64),
                        ),
                        false,
                    ),
            ...
            layer.write_text(
                &format!("HERRSCHVERMERK ZU LFD. NR. {}", unhyphenate(&s)),
                &fonts.helvetica,
            );
        }
        ...
        if self.geroetet.hat_links_rot() || self.geroetet.hat_rechts_rot() {
            let self_height = self.get_height_mm();
                    ...
                    if self_height == EXTENT_PER_LINE {
                        layer.add_shape(Line {
                            points: vec![
                                ...
                                (
                                    Point::new(
                                        Mm((x_start_mm + max_width_mm) as f64),
                                        Mm(y_start as f64),
                                    ),
                                    false,
                                ),
                        ...
                        layer.add_shape(Line {
                            points: vec![
                                (
                                    Point::new(
                                        Mm(x_start_mm as f64),
                                        Mm((y_start + EXTENT_PER_LINE) as f64),
                                    ),
                                    false,
                                ),
                                (
                                ...
                                (
                                    Point::new(
                                        Mm(x_start_mm as f64),
                                        Mm(((y_start + EXTENT_PER_LINE) - self_height - 1.0)
                                            as f64),
                                    ),
                                    false,
                                ),
                    ...
                    if self_height == EXTENT_PER_LINE {
                        layer.add_shape(Line {
                            points: vec![
                                ...
                                (
                                    Point::new(
                                        Mm(x_start_mm as f64 + (max_width_mm as f64 / 2.0)),
                                        Mm(y_start as f64),
                                    ),
                                    false,
                                ),
                    ...
                    if self_height == EXTENT_PER_LINE {
                        layer.add_shape(Line {
                            points: vec![
                                (
                                    Point::new(
                                        Mm(x_start_mm as f64 + (max_width_mm as f64 / 2.0) as f64),
                                        Mm(y_start as f64),
                                    ),
                                    false,
                                ),
...
impl PdfHeader {
    pub fn get_max_col_width(&self, col_id: usize) -> usize {
        ...
        let width = ((if col_id == spalten_lines.len() - 1 {
            ...
            self.get_starting_x_spalte_mm(col_id + 1) - self.get_starting_x_spalte_mm(col_id)
        } / EXTENT_PER_LINE_X)
            .floor() as usize)
            .max(3);
        width
        ...
        let letzte_spalte_x = self
            ...
            .map(|last| {
                last.points
                    ...
                    .max()
                    .unwrap_or((start_erste_spalte * 1000.0) as usize) as f32
                    / 1000.0
            })
    ...
    fn add_to_page(&self, layer: &mut PdfLayerReference, fonts: &PdfFonts) {
        ...
        match self {
            PdfHeader::Bestandsverzeichnis => {
                ...
                let text_1 = &[("1", 17.0_f64, 297.0_f64 - 34.5)];
                ...
                let text_2 = &[("2", 32.0_f64, 297.0_f64 - 34.5)];
                ...
                let text_3 = &[("3", 100.0_f64, 297.0_f64 - 34.5)];
                ...
                let text_4 = &[("Größe", (210.0 - 28.0) as f64, 297.0_f64 - 21.0)];
                ...
                let text_4 = &[("4", (210.0 - 25.5) as f64, 297.0_f64 - 34.5)];
            ...
            PdfHeader::BestandsverzeichnisZuAb => {
                let text_1 = &[("Bestand und Zuschreibungen", 50.0_f64, 297.0_f64 - 20.7)];
                ...
                let text_1 = &[("5", 17.0_f64, 297.0_f64 - 34.5)];
                ...
                let text_2 = &[("6", 60.0_f64, 297.0_f64 - 34.5)];
                ...
                let text_1 = &[("Abschreibungen", 150.0_f64, 297.0_f64 - 20.7)];
                ...
                let text_1 = &[("7", 112.0_f64, 297.0_f64 - 34.5)];
                ...
                let text_2 = &[("8", 155.0_f64, 297.0_f64 - 34.5)];
            ...
            PdfHeader::Abteilung1 => {
                ...
                let text_1 = &[("Eigentümer", 55.0_f64, 297.0_f64 - 26.0)];
                ...
                let text_2 = &[("2", 60.0_f64, 297.0_f64 - 34.5)];
                ...
                let text_1 = &[("3", 112.0_f64, 297.0_f64 - 34.5)];
                ...
                let text_1 = &[("Grundlage der Eintragungen", 147.5_f64, 297.0_f64 - 26.0)];
                ...
                let text_2 = &[("4", 160.0_f64, 297.0_f64 - 34.5)];
            ...
            PdfHeader::Abteilung1ZuAb => {
                let text_1 = &[("Veränderungen", 60.0_f64, 297.0_f64 - 20.7)];
                ...
                let text_1 = &[("Löschungen", 155.0_f64, 297.0_f64 - 20.7)];
            ...
            PdfHeader::Abteilung2 => {
                ...
                let text_1 = &[("Lasten und Beschränkungen", 105.5_f64, 297.0_f64 - 26.0)];

                for (t, x, y) in text_1.iter() {
                    layer.use_text(*t, 6.0, Mm(*x), Mm(*y), &fonts.helvetica);
                }

                let text_2 = &[("3", 120.0_f64, 297.0_f64 - 34.5)];
            ...
            PdfHeader::Abteilung2ZuAb => {
                let text_1 = &[("Veränderungen", 50.0_f64, 297.0_f64 - 20.7)];
                ...
                let text_1 = &[("4", 17.0_f64, 297.0_f64 - 34.5)];
                ...
                let text_2 = &[("5", 60.0_f64, 297.0_f64 - 34.5)];
                ...
                let text_1 = &[("Löschungen", 145.0_f64, 297.0_f64 - 20.7)];
                ...
                let text_1 = &[("6", 112.0_f64, 297.0_f64 - 34.5)];
                ...
                let text_2 = &[("7", 155.0_f64, 297.0_f64 - 34.5)];
            ...
            PdfHeader::Abteilung3 => {
                ...
                let text_1 = &[("Betrag", 60.5_f64, 297.0_f64 - 26.0)];
                ...
                let text_2 = &[("3", 62.0_f64, 297.0_f64 - 34.5)];
                ...
                let text_2 = &[("4", 135.0_f64, 297.0_f64 - 34.5)];
...
fn get_text_rows(grundbuch: &Grundbuch, options: &PdfGrundbuchOptions) -> Vec<PdfTextRow> {
    ...
    if options.exportiere_bv {
        for bv in grundbuch.bestandsverzeichnis.eintraege.iter() {
            if !mit_geroeteten_eintraegen && bv.ist_geroetet() {
                continue;
            }
            match bv {
                BvEintrag::Flurstueck(flst) => {
                    ...
                    rows.push(PdfTextRow {
                        texts: vec![
                            format!("{}", flst.lfd_nr),
                            flst.bisherige_lfd_nr
                                .clone()
                                .map(|b| format!("{}", b))
                                .unwrap_or_default(),
                            flst.gemarkung
                                .clone()
                                .map(|g| if g == grundbuch_von { String::new() } else { g })
                                .unwrap_or_default(),
                            format!("{}", flst.flur),
        ...
        for i in 0..zu_ab_len {
            rows.push(PdfTextRow {
                texts: vec![
                    zuschreibungen
                        .get(i)
                        .map(|bvz| bvz.bv_nr.text())
                        .unwrap_or_default(),
                    zuschreibungen
                        .get(i)
                        .map(|bvz| bvz.text.text())
                        .unwrap_or_default(),
                    abschreibungen
                        .get(i)
                        .map(|bvz| bvz.bv_nr.text())
        ...
        for i in 0..zu_ab_len {
            rows.push(PdfTextRow {
                texts: vec![
                    eintraege
                        .get(i)
                        .map(|bvz| format!("{}", bvz.get_lfd_nr()))
                        .unwrap_or_default(),
                    eintraege
                        .get(i)
                        .map(|bvz| bvz.get_eigentuemer())
                        .unwrap_or_default(),
                    grundlagen_eintragungen
                        .get(i)
                        .map(|bvz| bvz.bv_nr.text())
    ...
    if options.exportiere_abt2 {
        for abt2 in grundbuch.abt2.eintraege.iter() {
            if !mit_geroeteten_eintraegen && abt2.ist_geroetet() {
                continue;
            }
    ...
    if options.exportiere_abt3 {
        for abt3 in grundbuch.abt3.eintraege.iter() {
            if !mit_geroeteten_eintraegen && abt3.ist_geroetet() {
                continue;
            }
...
fn render_text_rows(doc: &mut PdfDocumentReference, fonts: &PdfFonts, blocks: &[PdfTextRow]) {
    ...
    current_block
        .header
        .add_to_page(&mut doc.get_page(page).get_layer(layer), fonts);
    current_block
        .header
        .add_columns_to_page(&mut doc.get_page(page).get_layer(layer));
    ...
    for b in blocks.iter().skip(1) {
        if b.header != current_header
            || current_y - b.get_height_mm() - 2.0 < current_header.get_end_y()
        {
            ...
            b.header
                .add_to_page(&mut doc.get_page(new_page).get_layer(new_layer), fonts);
            b.header
                .add_columns_to_page(&mut doc.get_page(new_page).get_layer(new_layer));
            page = new_page;
            layer = new_layer;
        }
...
pub fn hyphenate(text: &str, wrap_at_chars: usize) -> Vec<String> {
    let options =
        Options::new(wrap_at_chars).word_splitter(WordSplitter::Hyphenation(DICT_DE.clone()));
