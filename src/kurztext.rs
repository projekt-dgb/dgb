...
lazy_static! {
    static ref EINGETRAGEN_AM_REGEX: Regex =
        Regex::new(r"ingetragen am (\d\d).(\d\d).(\d\d\d\d)").unwrap();
    static ref UEBERTRAGEN_AM_REGEX: Regex =
        Regex::new(r"hierher übertragen am (\d\d).(\d\d).(\d\d\d\d)").unwrap();
    static ref EINGETRAGEN_AM_REGEX_2: Regex =
        Regex::new(r"ingetragen (.*) am (\d\d).(\d\d).(\d\d\d\d)").unwrap();
}
...
pub fn formatiere_betrag(b: &Betrag) -> String {
    ...
    if b.wert >= 1_000_000 {
        ...
        format!(
            "{}.{:03}.{:03},{:02}",
            millionen, hunderttausender, letzte_drei_stellen, b.nachkomma
        )
    } else if b.wert >= 100_000 {
        let hunderttausender = hunderttausender % 1000;
        format!(
            "{:03}.{:03},{:02}",
            hunderttausender, letzte_drei_stellen, b.nachkomma
        )
    } else if b.wert >= 1000 {
        let hunderttausender = hunderttausender % 1000;
        format!(
...
pub fn text_saubern(
    ...
    for (s_idx, s) in saetze.iter().enumerate() {
        let naechster_satz_faengt_mit_großbuchstaben_an = saetze
            .get(s_idx + 1)
            .and_then(|s| s.trim().chars().nth(0))
            .map(|fc| fc.is_uppercase())
            .unwrap_or(false);
        let naechster_satz_ist_monat = saetze
            .get(s_idx + 1)
            .map(|s| MONATE.iter().any(|m| s.trim().starts_with(m)))
            .unwrap_or(false);
